...
class RunnerV3(object):
    def __init__(self, model, optimizer, loss_fn, metric, device=None, **kwargs):
        self.device = device or ('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = model.to(self.device)  # 将模型转移到GPU或CPU
    ...
    def evaluate(self, dev_loader, **kwargs):
        ...
        for batch_id, data in enumerate(dev_loader):
            X, y = data
            X, y = X.to(self.device), y.to(self.device)  # 将数据移到GPU
